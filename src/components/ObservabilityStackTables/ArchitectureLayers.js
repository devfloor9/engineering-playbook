...
const ArchitectureLayers = () => {
  ...
  return <div style={containerStyle}>
      ...
      <div style={layersContainerStyle}>
        {layers.map((layer, index) => <React.Fragment key={index}>
            <div style={layerStyle(layer.color)}>
              ...
              <div style={componentsStyle}>
                {layer.components.map((component, idx) => <span key={idx} style={componentTagStyle}>{component}</span>)}
              </div>
            </div>
            {index < layers.length - 1 && <div style={arrowStyle}>↓</div>}
          </React.Fragment>)}
      </div>
    </div>;
};
