...
export default function SolutionFeatureTable({ solution, color, features, limitations, locale = 'en' }) {
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', maxWidth: 760, margin: '0 0 1.5rem 0' }}>
      ...
      <div style={{ background: 'var(--ifm-background-surface-color)', border: '1px solid var(--ifm-color-emphasis-200)', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '0.75rem' }}>
        {/* Features Section */}
        <div style={{ marginBottom: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {features.map((item, idx) => (
              <div key={idx} style={{ border: '1.5px solid #16a34a30', borderLeft: '4px solid #16a34a', borderRadius: 8, padding: '0.7rem 1rem', background: 'var(--ifm-color-emphasis-100)' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#15803d' }}>{item.feature}</span>
                  <span style={{ fontSize: '0.76rem', color: 'var(--ifm-font-color-base)' }}>{item.benefit}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {limitations.map((item, idx) => (
              <div key={idx} style={{ border: '1.5px solid #dc262630', borderLeft: '4px solid #dc2626', borderRadius: 8, padding: '0.7rem 1rem', background: 'var(--ifm-color-emphasis-100)' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#991b1b' }}>{item.limitation}</span>
                  <span style={{ fontSize: '0.76rem', color: 'var(--ifm-font-color-base)' }}>{item.impact}</span>
                </div>
              </div>
            ))}
          </div>
