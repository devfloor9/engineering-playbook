...
const difficultyStyles = {
  ko: {
    '쉬움': { bg: 'var(--ifm-color-emphasis-100)', border: '#4caf50', color: '#2e7d32' },
    '중간': { bg: '#fff3e0', border: '#ff9800', color: '#e65100' },
    '어려움': { bg: '#ffebee', border: '#f44336', color: '#c62828' },
  },
  en: {
    'Easy': { bg: 'var(--ifm-color-emphasis-100)', border: '#4caf50', color: '#2e7d32' },
    ...
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', maxWidth: '100%', margin: '0 0 1.5rem 0' }}>
      ...
      <div style={{ background: 'var(--ifm-background-surface-color)', border: '1px solid var(--ifm-color-emphasis-200)', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '0.75rem', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
          <thead>
            <tr>
              <th style={{ padding: '0.6rem 0.8rem', textAlign: 'left', background: 'var(--ifm-color-emphasis-100)', borderBottom: '2px solid var(--ifm-color-emphasis-200)', fontWeight: 700, fontSize: '0.76rem', color: 'var(--ifm-font-color-base)', minWidth: '120px' }}>{locale === 'ko' ? '기능' : 'Feature'}</th>
              <th style={{ padding: '0.6rem 0.8rem', textAlign: 'center', background: 'var(--ifm-color-emphasis-100)', borderBottom: '2px solid #ff9800', fontWeight: 700, fontSize: '0.74rem', color: 'var(--ifm-font-color-base)', minWidth: '120px' }}>AWS Native</th>
              <th style={{ padding: '0.6rem 0.8rem', textAlign: 'center', background: 'var(--ifm-color-emphasis-100)', borderBottom: '2px solid #2196f3', fontWeight: 700, fontSize: '0.74rem', color: 'var(--ifm-font-color-base)', minWidth: '120px' }}>Cilium</th>
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr key={idx} style={{ background: idx % 2 === 0 ? 'var(--ifm-color-emphasis-100)' : '#fff' }}>
                <td style={{ padding: '0.7rem 0.8rem', fontWeight: 600, color: '#212121', borderBottom: '1px solid var(--ifm-color-emphasis-200)' }}>{item.feature}</td>
                <td style={{ padding: '0.7rem 0.8rem', borderBottom: '1px solid var(--ifm-color-emphasis-200)' }}>{renderCell(item.aws)}</td>
                <td style={{ padding: '0.7rem 0.8rem', borderBottom: '1px solid var(--ifm-color-emphasis-200)' }}>{renderCell(item.cilium)}</td>
