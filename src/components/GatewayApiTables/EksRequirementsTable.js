...
const badgeColors = {
  version: { bg: 'var(--ifm-color-emphasis-100)', text: '#1565c0' },
    ...
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', maxWidth: 760, margin: '0 0 1.5rem 0' }}>
      ...
      <div style={{ background: 'var(--ifm-background-surface-color)', border: '1px solid var(--ifm-color-emphasis-200)', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {items.map((row, idx) => {
          ...
          return (
            <div key={idx} style={{ border: '1.5px solid #fb8c0020', borderLeft: '4px solid #f57c00', borderRadius: 8, padding: '0.7rem 1rem', background: 'var(--ifm-color-emphasis-100)' }}>
              ...
              <div style={{ fontSize: '0.76rem', color: 'var(--ifm-color-emphasis-600)', fontStyle: 'italic' }}>{row.notes}</div>
            </div>
          );
        })}
      </div>
