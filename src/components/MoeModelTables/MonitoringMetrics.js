...
const MonitoringMetrics = () => {
  ...
  const metrics = [{
    ...
    description: isKo ? '현재 처리 중인 요청 수' : 'Current requests being processed',
    threshold: '-',
    color: '#3b82f6',
    severity: 'info'
  }, {
    metric: 'vllm:num_requests_waiting',
    description: isKo ? '대기 중인 요청 수' : 'Requests waiting in queue',
    threshold: '> 100 ' + (isKo ? '경고' : 'warning'),
    color: '#f59e0b',
    severity: 'warning'
  }, {
    metric: 'vllm:gpu_cache_usage_perc',
    description: isKo ? 'KV Cache 사용률' : 'KV Cache utilization',
    threshold: '> 95% ' + (isKo ? '경고' : 'warning'),
    color: '#ef4444',
    severity: 'critical'
  }, {
    metric: 'vllm:avg_prompt_throughput_toks_per_s',
    description: isKo ? '프롬프트 처리량' : 'Prompt processing throughput',
    threshold: '-',
    color: '#10b981',
    severity: 'info'
  }, {
    metric: 'vllm:avg_generation_throughput_toks_per_s',
    description: isKo ? '생성 처리량' : 'Generation throughput',
    threshold: '-',
    color: '#8b5cf6',
    severity: 'info'
  }, {
    ...
    fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'
      ...
      borderRadius: '8px 8px 0 0'
        ...
        fontWeight: '600'
          ...
          {isKo ? 'MoE 모델 주요 모니터링 메트릭' : 'MoE Model Key Monitoring Metrics'}
        </div>
      </div>
