    ...
    fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'
      ...
      borderRadius: '8px 8px 0 0'
        ...
        fontWeight: '600'
          ...
          {isKo ? 'vLLM 텐서 병렬화 권장 설정' : 'vLLM Tensor Parallelism Recommended Configuration'}
        </div>
      </div>
      ...
      overflow: 'hidden'
        ...
        borderBottom: '2px solid var(--ifm-color-emphasis-300)'
          ...
          <div>{isKo ? '모델' : 'Model'}</div>
          <div>{isKo ? '권장 TP 크기' : 'Recommended TP Size'}</div>
          <div>{isKo ? 'GPU 구성' : 'GPU Configuration'}</div>
          <div>{isKo ? '메모리/GPU' : 'Memory/GPU'}</div>
        </div>

        {configs.map((config, index) => <div key={index} style={{
        display: 'grid',
        gridTemplateColumns: '200px 120px 200px 150px',
        padding: '16px 20px',
