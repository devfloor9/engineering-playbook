...
const MonitoringArchitecture = () => {
  ...
  const approaches = [{
    ...
    badge: 'Managed OSS',
    badgeColor: '#059669',
    description: isKo ? 'ADOT Collector / Prometheus로 CoreDNS 메트릭 스크랩 → AMP remote write → Grafana(AMG) 시각화' : 'ADOT Collector / Prometheus scrapes CoreDNS metrics → AMP remote write → Grafana (AMG) visualization',
    pros: isKo ? ['PromQL 네이티브 쿼리', '장기 보관 & 대규모 클러스터 지원', 'Terraform 자동화 가속기 제공'] : ['Native PromQL queries', 'Long-term retention & large-scale support', 'Terraform automation accelerator'],
    cons: isKo ? ['ADOT/Prometheus 설치 필요', '수집 메트릭 기반 요금'] : ['Requires ADOT/Prometheus installation', 'Charges based on ingested metrics']
  }, {
  ...
  const layers = [{
    ...
    name: isKo ? '시각화 (Visualization)' : 'Visualization',
    color: '#f59e0b',
    items: ['AMG (Grafana)', 'CloudWatch Dashboards']
  }, {
    name: isKo ? '알림 (Alerting)' : 'Alerting',
    color: '#ef4444',
    ...
    padding: '0 1rem'
      ...
      borderRadius: '8px 8px 0 0'
        ...
        marginBottom: '4px'
          ...
          {isKo ? '🏗️ CoreDNS 모니터링 아키텍처' : '🏗️ CoreDNS Monitoring Architecture'}
        </div>
        <div style={{
        fontSize: '14px',
        opacity: 0.9
      }}>
          {isKo ? 'AMP + ADOT vs CloudWatch Container Insights 비교' : 'AMP + ADOT vs CloudWatch Container Insights Comparison'}
        </div>
      </div>
      ...
      overflow: 'hidden'
        ...
        {/* Approach cards */}
        {approaches.map((approach, idx) => <div key={idx} style={{
        padding: '16px 20px',
        borderBottom: '1px solid #f3f4f6'
          ...
          flexWrap: 'wrap'
              ...
              textTransform: 'uppercase'
                ...
                {approach.pros.map((p, i) => <div key={i} style={{
              fontSize: '12px',
              color: 'var(--ifm-font-color-base)',
              lineHeight: '1.6'
            }}>+ {p}</div>)}
              ...
              textTransform: 'uppercase'
                  ...
                  {isKo ? '고려사항' : 'Considerations'}
                </div>
                {approach.cons.map((c, i) => <div key={i} style={{
              fontSize: '12px',
        ...
        padding: '16px 20px'
          ...
          textTransform: 'uppercase'
            ...
            {isKo ? '파이프라인 레이어' : 'Pipeline Layers'}
          </div>
          {layers.map((layer, idx) => <React.Fragment key={idx}>
              <div style={{
