...
const GoldenSignals = () => {
  ...
  const signals = [{
    icon: '📈',
    name: isKo ? '트래픽 (Throughput)' : 'Throughput',
    metric: 'coredns_dns_requests_total',
    description: isKo ? '초당 DNS 요청 수(QPS). Pod별 부하 균등 여부를 확인하고, 지속 증가 시 스케일 아웃을 검토합니다.' : 'DNS queries per second (QPS). Check per-Pod load balance; consider scale-out on sustained growth.',
    color: '#3b82f6'
  }, {
    icon: '⏱️',
    name: isKo ? '지연 시간 (Latency)' : 'Latency',
    metric: 'coredns_dns_request_duration_seconds',
    description: isKo ? 'P99 응답 시간. 평소보다 높아지면 업스트림 DNS 지연 또는 CoreDNS CPU/메모리 포화를 점검합니다.' : 'P99 response time. If elevated, check upstream DNS latency or CoreDNS CPU/memory saturation.',
    color: '#f59e0b'
  }, {
    icon: '❌',
    ...
    padding: '0 1rem'
      ...
      borderRadius: '8px 8px 0 0'
        ...
        marginBottom: '4px'
          ...
          {isKo ? '🎯 CoreDNS 4 Golden Signals' : '🎯 CoreDNS 4 Golden Signals'}
        </div>
        <div style={{
        fontSize: '14px',
        opacity: 0.9
      }}>
          {isKo ? 'Google SRE 방법론 기반 핵심 모니터링 지표' : 'Core monitoring indicators based on Google SRE methodology'}
        </div>
      </div>
      ...
      overflow: 'hidden'
        ...
        {signals.map((signal, idx) => <div key={idx} style={{
        display: 'flex',
        gap: '16px',
        padding: '16px 20px',
