  ...
  return <div style={{
    ...
    lineHeight: '1.6'
      ...
      overflow: 'hidden'
        ...
        {costItems.map((item, idx) => <div key={item.name} style={{
        display: 'grid',
        gridTemplateColumns: '140px 140px 1fr',
        ...
        color: 'var(--ifm-color-emphasis-700)'
          ...
          <strong>{isKo ? '비용 최적화 전략:' : 'Cost Optimization Strategy:'}</strong> {isKo ? '초기에는 전체 활성화로 가치를 검증한 후, 데이터 기반으로 불필요한 메트릭과 로그를 필터링하여 비용을 점진적으로 최적화하세요. AWS Cost Explorer와 CloudWatch Contributor Insights로 비용 구조를 분석할 수 있습니다.' : 'Initially validate value with full activation, then gradually optimize costs by filtering unnecessary metrics and logs based on data. Analyze cost structure using AWS Cost Explorer and CloudWatch Contributor Insights.'}
        </div>
      </div>
    </div>;
};
