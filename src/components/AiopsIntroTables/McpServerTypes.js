...
const McpServerTypes = () => {
  ...
  const rows = [{
    label: isKo ? '출시' : 'Release',
    key: 'released'
  }, {
    label: isKo ? '실행 위치' : 'Location',
    key: 'location'
  }, {
    label: isKo ? '범위' : 'Scope',
    key: 'scope'
  }, {
    label: isKo ? '핵심 특징' : 'Key Features',
    key: 'feature'
  }, {
    label: isKo ? '설치/연결' : 'Install/Connect',
    key: 'install'
  }, {
    label: isKo ? '활용 시나리오' : 'Use Case',
    key: 'useCase'
  }];
  return <div style={{
    maxWidth: '760px',
    ...
    lineHeight: '1.6'
      ...
      overflow: 'hidden'
        ...
        borderBottom: '2px solid var(--ifm-color-emphasis-200)'
          ...
          {types.map(t => <div key={t.name} style={{
          padding: '12px 14px',
        ...
        borderBottom: idx < rows.length - 1 ? '1px solid #f3f4f6' : 'none'
          ...
          alignItems: 'center'
            ...
            {types.map(t => <div key={`${row.key}-${t.name}`} style={{
          padding: '10px 14px',
          fontSize: '12px',
          color: 'var(--ifm-font-color-base)',
        ...
        color: 'var(--ifm-color-emphasis-700)'
          ...
          <strong>{isKo ? '권장 시작점:' : 'Recommended Start:'}</strong> {isKo ? '개별 로컬 MCP 서버(GA)로 시작하여 Kiro+MCP 패턴을 검증한 후, 엔터프라이즈 보안 요구사항에 따라 Fully Managed로 전환하세요. AWS MCP Server(통합)는 멀티 서비스 복합 작업에 적합합니다.' : 'Start with Individual Local MCP Server (GA) to validate Kiro+MCP patterns, then migrate to Fully Managed based on enterprise security requirements. AWS MCP Server (Unified) is ideal for multi-service complex tasks.'}
        </div>
      </div>
    </div>;
};
