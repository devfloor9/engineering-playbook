...
const OperationPatternsComparison = () => {
  ...
  const patterns = [{
    ...
    responseStart: isKo ? '运营자가 알림 확인 후 AI에 지시' : 'Operator checks alert, then directs AI',
  ...
  const rows = [{
    label: isKo ? '사람의 역할' : 'Human Role',
    key: 'humanRole'
  }, {
    label: isKo ? '대응 시작' : 'Response Start',
    key: 'responseStart'
  }, {
    label: isKo ? '데이터 수집' : 'Data Collection',
    key: 'dataCollection'
  }, {
    label: isKo ? '분석' : 'Analysis',
    key: 'analysis'
  }, {
    label: isKo ? '복구' : 'Recovery',
    key: 'recovery'
  }, {
    label: isKo ? '학습' : 'Learning',
    key: 'learning'
  }, {
    label: isKo ? '대응 시간' : 'Response Time',
    key: 'responseTime'
  }, {
    label: isKo ? '대표 도구' : 'Representative Tools',
    key: 'tools'
  }];
  return <div style={{
    maxWidth: '760px',
    ...
    lineHeight: '1.6'
      ...
      overflow: 'hidden'
        ...
        borderBottom: '2px solid var(--ifm-color-emphasis-200)'
          ...
          {patterns.map(p => <div key={p.name} style={{
          padding: '12px 14px',
        ...
        {rows.map((row, idx) => <div key={row.key} style={{
        display: 'grid',
        gridTemplateColumns: '110px 1fr 1fr 1fr',
        borderBottom: idx < rows.length - 1 ? '1px solid #f3f4f6' : 'none'
          ...
          alignItems: 'center'
            ...
            {patterns.map(p => <div key={`${row.key}-${p.name}`} style={{
          padding: '10px 14px',
          fontSize: '12px',
          color: 'var(--ifm-font-color-base)',
        ...
        color: 'var(--ifm-color-emphasis-700)'
          ...
          <strong>{isKo ? '실전 조합:' : 'Real-world Combination:'}</strong> {isKo ? '세 패턴은 상호 보완적입니다. 새로운 장애를 Prompt-Driven으로 탐색한 뒤, 반복 패턴을 Spec-Driven으로 코드화하고, 최종적으로 Agent-Driven으로 자율화하는 점진적 성숙 과정을 거칩니다.' : 'The three patterns are complementary. Explore new failures with Prompt-Driven, codify recurring patterns with Spec-Driven, and finally automate with Agent-Driven in a gradual maturity process.'}
        </div>
      </div>
    </div>;
};
