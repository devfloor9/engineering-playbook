...
The [correction ledger](audits/2026-09-19/corrections.json) records subsequent reviewed changes and partial fixes without rewriting the original findings. Recent corrections cover lifecycle and scheduling behavior, Gateway API examples, private networking, distributed serving, model releases and prompt registries. The fifth follow-up completes the registered Deployment-selector, Istio outlier/routing and Karpenter-budget corrections. A separate [IP capacity and CPU sizing review](audits/2026-09-19/pr104-follow-up.md) covers 13 findings in documents added or changed after the frozen inventory. Offline regression checks exercise the documented examples; runtime and performance claims require separate evidence.
