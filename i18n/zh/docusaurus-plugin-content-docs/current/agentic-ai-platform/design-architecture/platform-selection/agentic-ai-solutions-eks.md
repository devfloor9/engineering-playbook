...
:::info 前置文档
阅读本文之前请先参阅以下文档：
- [平台架构](../foundations/agentic-platform-architecture.md) — Agentic AI Platform 的结构与核心层级
- [技术挑战](../foundations/agentic-ai-challenges.md) — 5 大核心挑战
...
详细配置请参阅 [vLLM 模型服务](../../model-serving/inference-frameworks/vllm-model-serving.md) 和 [llm-d 分布式推理](../../model-serving/inference-frameworks/llm-d-eks-automode.md)。
:::
...
> 详细内容请参阅 [Kagent Agent 管理](../../operations-mlops/observability/kagent-kubernetes-agents.md) 和 [AWS Native 平台 — AgentCore & MCP](./aws-native-agentic-platform.md#mcp-协议与-eks-集成)。

#### RAG + 可观测性

- **Milvus**：向量数据库 — RAG 系统核心（[详情](../../operations-mlops/data-infrastructure/milvus-vector-database.md)）
...
:::info GPU 成本优化详情
Spot 实例利用、Consolidation、基于时段的调度成本管理等 GPU 成本优化策略请参阅 [GPU 资源管理](../../model-serving/gpu-infrastructure/gpu-resource-management.md) 文档。
:::

:::info GPU 安全与故障排除
GPU Pod 安全策略、Network Policy、IAM、MIG 隔离及 GPU 故障排除指南请参阅 [EKS GPU 节点策略](../../model-serving/gpu-infrastructure/eks-gpu-node-strategy.md) 文档。
:::
...
**推荐混合配置**：在单一集群中运营 Auto Mode（一般工作负载）+ Karpenter（GPU 高级功能）。详细配置请参阅 [EKS GPU 节点策略](../../model-serving/gpu-infrastructure/eks-gpu-node-strategy.md)。
...
| 领域 | 文档 | 内容 |
|------|------|------|
| GPU 节点策略 | [EKS GPU 节点策略](../../model-serving/gpu-infrastructure/eks-gpu-node-strategy.md) | Auto Mode + Karpenter + Hybrid Node + 安全/故障排除 |
| GPU 资源管理 | [GPU 资源管理](../../model-serving/gpu-infrastructure/gpu-resource-management.md) | Karpenter 伸缩、KEDA、DRA、成本优化 |
| NVIDIA GPU 栈 | [NVIDIA GPU 栈](../../model-serving/gpu-infrastructure/nvidia-gpu-stack.md) | GPU Operator、DCGM、MIG、Time-Slicing |
| 模型服务 | [vLLM 模型服务](../../model-serving/inference-frameworks/vllm-model-serving.md) | vLLM 配置、性能优化 |
| 分布式推理 | [llm-d 分布式推理](../../model-serving/inference-frameworks/llm-d-eks-automode.md) | KV Cache 感知路由 |
| 训练基础设施 | [NeMo 框架](../../model-serving/inference-frameworks/nemo-framework.md) | 分布式训练、EFA 网络 |
