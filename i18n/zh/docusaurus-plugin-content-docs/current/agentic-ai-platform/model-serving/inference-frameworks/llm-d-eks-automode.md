...
:::tip 实战部署指南
llm-d 的 EKS 部署 YAML、helmfile 命令、集群创建等实战部署请参阅 [自定义模型部署指南](../../reference-architecture/model-lifecycle/custom-model-deployment.md)。
:::
...
按模型大小的详细成本分析请参阅 [EKS GPU 节点策略](../gpu-infrastructure/eks-gpu-node-strategy.md)。
...
- [EKS GPU 节点策略](../gpu-infrastructure/eks-gpu-node-strategy.md) -- Auto Mode vs Karpenter vs Hybrid Node，按模型大小成本分析
- [vLLM 模型服务及性能优化](./vllm-model-serving.md) -- vLLM 基本概念及部署
- [MoE 模型服务指南](./moe-model-serving.md) -- Mixture of Experts 模型服务
- [GPU 资源管理](../gpu-infrastructure/gpu-resource-management.md) -- GPU 集群资源管理
