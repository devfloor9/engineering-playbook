...
:::info 自动模型分流
使用 LLM Classifier 时客户端通过单一端点（`/v1`）请求，根据 Prompt 内容自动选择 SLM/LLM。简单请求路由到 Qwen3-4B（L4 $0.3/hr），复杂请求（重构、架构、设计等）路由到 GLM-5 744B（H200 $12/hr）。配置请参阅 [网关配置指南](./inference-gateway-setup.md)。
:::
