...
description: Why the same workload shows different CPU utilization across instance sizes and generations (run queue, shared cache and memory bandwidth, SMT, CFS quota), which KPIs to compare instead, a Pod sizing baseline, and a procedure for deciding on mixed node pools.
created: "2026-09-19"
last_update:
  date: "2026-09-19"
...
Move the same container image to a node of a different size or generation and its CPU utilization (CPU%) often looks different. Domain teams read this as a regression and platform teams start wondering whether to roll back the node sizing policy, but CPU% is not a number that can be compared as-is across sizes and generations. This guide explains why CPU% shifts, which metrics to use when comparing sizes and generations, how to set a Pod sizing baseline, and how to decide whether several sizes or generations can share one workload. It is written for platform teams that set node sizing policy and for domain teams that run CPU% alarms.
...
CPU utilization changes meaning as it passes from physical hardware up to the Pod-level observation. Other workloads, settings, and the scheduler intervene at each layer, so the CPU% seen at the top does not reflect the state of the layers below.
...
Because `static` is a node-level kubelet setting, put it in a separate NodePool and schedule only the workloads that need exclusive cores there. `spec.kubelet` on a Karpenter v1 EC2NodeClass supports a subset of kubelet fields and `cpuManagerPolicy` is not among them. Keep the CPU reservation in `spec.kubelet` and pass the policy through AL2023 `NodeConfig` userData, which is merged with the NodeConfig Karpenter generates. NodePool syntax and weight are covered in [Karpenter Autoscaling](./karpenter-autoscaling.md).
...
CPU performance is observed through throttling, scheduling wait, and efficiency together, not utilization alone.

- Measure CFS throttling with cAdvisor's `container_cpu_cfs_periods_total`, `container_cpu_cfs_throttled_periods_total`, and `container_cpu_cfs_throttled_seconds_total` to get the throttled ratio and the missing quota. On cgroup v2, `nr_periods`, `nr_throttled`, and `throttled_usec` in `cpu.stat` carry the same information.
- Read run queue wait and context switches from cAdvisor's `container_cpu_schedstat_runqueue_seconds_total` (time the container spent waiting in the run queue) and `container_cpu_schedstat_run_seconds_total`. These belong to cAdvisor's `sched` metric group, which is disabled by default and not exposed by the kubelet's embedded endpoint, so a standalone cAdvisor DaemonSet with `-enable_metrics=sched` is needed. For fine-grained run queue latency histograms, add eBPF `runqlat`.
- CloudWatch Container Insights (Enhanced) provides `pod_cpu_utilization`, `pod_cpu_utilization_over_pod_limit`, `pod_cpu_reserved_capacity`, and similar, but no CFS throttled-period metric. Throttling observation needs the cAdvisor→Prometheus path ([Enhanced Container Insights metrics](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Container-Insights-metrics-enhanced-EKS.html)).
- 60-second averages hide sub-minute throttling spikes; use 15–30 second resolution and histograms.
...
- cAdvisor's CFS and schedstat counters are counter-type metrics: apply `rate()` first, then compute ratios. Keep namespace/pod/container identifiers rather than summing replicas into one value, and watch for time-series breaks from Pod recreation and duplicate scrapes. Reuse the PromQL from the [CPU throttling observation section of the Pod Resource Optimization Guide](./eks-resource-optimization.md#633-automatic-cpu-throttling-detection).
...
| Latency rises at low utilization | CFS global quota throttles a multithreaded process | Consider omitting the CPU limit or align thread counts | Watch the throttled_periods ratio |
...
CPU utilization is not a metric that can be compared directly across instance sizes and generations. Comparisons use sustained RPS, p99, and cost-per-1K-req measured under identical conditions. Pod sizing aligns the CPU:memory ratio to a family and sets a minimum vCPU floor; latency-sensitive workloads weigh the trade-off between Guaranteed integer CPU and CPU Manager static. CFS throttling is observed through the cAdvisor→Prometheus path because Container Insights has no native metric for it. Per-generation comparison is automated with a benchmark pipeline, and only the sizes and generations that meet the SLO stay in the node pool.
...
- [cAdvisor Prometheus metrics](https://github.com/google/cadvisor/blob/master/docs/storage/prometheus.md) — CFS and schedstat container metric definitions
