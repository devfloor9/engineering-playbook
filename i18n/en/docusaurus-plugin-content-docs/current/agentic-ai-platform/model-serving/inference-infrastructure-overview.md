---
title: Inference Infrastructure Overview and Tuning Layers
...
keywords:
  ...
  - LMCache
sidebar_label: Inference Infrastructure Overview
---
...
Definitions and the correspondence between the two layers are covered in detail in [Tiered Gateway Architecture](./inference-routing/tiered-gateway-architecture.md) and [Routing Strategy — Two Routing Layers](./inference-routing/routing-strategy.md#두-개의-라우팅-레이어--반드시-구분).
...
To overcome these limitations, a separate inference routing layer (Tier 2 ①) that is KV- and load-aware is required. For detailed rationale, see [KV Cache Optimization — Limitations of Round-Robin](./inference-optimization/kv-cache-optimization.md#기존-문제-round-robin의-한계) and [Routing Strategy — Two Routing Layers](./inference-routing/routing-strategy.md#두-개의-라우팅-레이어--반드시-구분).
...
For a detailed comparison, see [KV Cache Optimization — KV Cache-Aware Routing](./inference-optimization/kv-cache-optimization.md#kv-cache-aware-routing) and [Routing Strategy — Precise Definition of EPP](./inference-routing/routing-strategy.md#eppendpoint-picker-정확한-정의).
...
<DocCardGrid columns={3}>
  <DocCard
    ...
    description="Deep-dives on KV Cache · Disaggregated Serving · LMCache · cache-hit strategy and gateway routing."
    color="#00897b"
  />
  <DocCard
    ...
    description="vLLM PagedAttention · Multi-LoRA, llm-d distributed inference, MoE · NeMo."
    color="#ff6b6b"
  />
  <DocCard
    ...
    description="EKS GPU node strategy, Karpenter · KEDA · DRA, NVIDIA · Neuron stack."
    color="#326ce5"
  />
</DocCardGrid>
