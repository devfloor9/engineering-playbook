...
CoreDNS는 `metrics` 플러그인을 통해 **Prometheus 형식의 메트릭**를 제공하며, 기본적으로 EKS에서는 `kube-dns` 서비스의 `9153` 포트로 노출됩니다. 핵심 메트릭들은 **DNS 요청의 처리량, 지연 시간, 오류, 캐싱 효율** 등을 보여주며, 이를 모니터링함으로써 DNS 성능 병목이나 장애 징후를 빠르게 포착할 수 있습니다. 아래 **표 1**은 CoreDNS의 주요 Prometheus 메트릭과 그 의미 및 활용법을 정리한 것입니다.

**표 1. CoreDNS 주요 Prometheus 메트릭과 의미**

이 외에도 **요청/응답 크기**(`coredns_dns_request_size_bytes`, `...response_size_bytes`), **DO 비트 설정 여부**(`coredns_dns_do_requests_total`) 등의 메트릭가 제공되며, CoreDNS에 로드된 **플러그인별 추가 메트릭**도 존재할 수 있습니다. 예를 들어 **Forward 플러그인**을 통한 업스트림 질의 시간(`coredns_forward_request_duration_seconds`)이나 **kubernetes 플러그인**의 API 업데이트 지연(`coredns_kubernetes_dns_programming_duration_seconds`) 등이 있습니다. 이러한 메트릭들을 종합적으로 모니터링하면 CoreDNS의 **4가지 골든 시그널**을 관리할 수 있습니다.
...
### 참고해야 할 추가 사항(ENI DNS 패킷 제한)

:::warning VPC ENI DNS 패킷 제한
각 노드 ENI는 초당 1024개의 DNS 패킷만 허용합니다. CoreDNS의 1000 동시 질의 한계를 풀어도, ENI PPS 한계(1024 PPS)의 제한으로 인하여 원하는 성능에 도달하지 못할 수도 있습니다.
:::
...
Amazon EKS에서는 CoreDNS의 Prometheus 메트릭를 수집하기 위해 **두 가지 접근**이 일반적입니다:

1. **Amazon Managed Service for Prometheus (AMP)**: AWS에서 제공하는 **완전 관리형 Prometheus 호환** 서비스로, 클러스터 내 메트릭를 원격 수집(remote write)하여 **확장성 높은 시계열 DB**에 보관합니다. EKS 클러스터에는 **ADOT(AWS Distro for OpenTelemetry) Collector** 또는 **Prometheus 서버**를 설치하여 CoreDNS 메트릭를 스크랩한 후 AMP로 전송합니다. AWS Observability 가이드에서는 Terraform 기반으로 ADOT Collector 및 AMP 워크스페이스를 생성/구성하여 EKS 모니터링을 자동화할 수 있는 **가속기(Accelerator)**도 제공합니다. AMP에 저장된 메트릭은 **PromQL**로 쿼리 가능하며, 장기 보관 및 대규모 클러스터 지원에 적합합니다.

2. **CloudWatch Container Insights (및 CloudWatch 에이전트):** AWS의 CloudWatch를 활용하여 **Prometheus 메트릭을 CloudWatch로 수집**하는 방법입니다. CloudWatch 에이전트를 DaemonSet으로 배포하고, `kube-system/kube-dns` 서비스의 9153 포트로부터 CoreDNS 메트릭를 스크랩하도록 설정합니다. AWS 공식 문서에서는 Prometheus 호환 메트릭를 가져오기 위한 CloudWatch Agent 설정 예제를 제공하며, 이렇게 수집된 메트릭은 CloudWatch 콘솔이나 CloudWatch Metrics로 보관됩니다. CloudWatch는 메트릭에 대한 **경보(Alarm)** 연계가 용이하고, AWS 관리형 Grafana를 통해 시각화할 수도 있습니다. 다만 **CloudWatch 요금** (메트릭 수집/저장)이 발생하고, PromQL을 직접 사용하기보다는 정해진 메트릭로 CloudWatch 대시보드를 구성하는 형태가 됩니다.
...
CoreDNS의 **쿼리 로그와 에러 로그**는 성능 문제를 진단하거나 보안 모니터링(예: 특정 도메인에 대한 폭주 조회) 측면에서 유용한 정보源입니다. CoreDNS의 기본 Corefile에는 `log` 플러그인이 없지만, 필요에 따라 `log` 또는 `errors` 플러그인을 활성화할 수 있습니다. **실무에서는** CoreDNS Pod의 표준 출력(stdout/stderr)에 기록되는 로그를 수집하기 위해 **Fluent Bit**이나 **Fluentd**를 DaemonSet으로 운용하여 CloudWatch Logs로 내보내는 패턴이 흔합니다.
...
- **HighDNSLatency**: `coredns_dns_request_duration_seconds`의 **p99 지연 시간**이 예를 들어 **500ms**를 초과하고 평소보다 높을 때 경보.
- **DNSErrorsSpike**: `coredns_dns_responses_total`에서 `rcode` 라벨이 `SERVFAIL` 또는 `NXDOMAIN`인 값의 비율이 일정 임계치 이상일 때 경보.
- **ENIThrottling**: AWS 환경 특화 메트릭로, **EC2 네트워크 인터페이스(ENI)의 DNS 패킷 제한 초과**를 모니터링하는 경보입니다.
...
*사례 1*: 한 기업에서 EKS 기반 마이크로서비스 아키텍처를 운영 중, 특정 서비스의 DNS 응답이 지연되며 **전체 응답시간이 1초 이상 추가되는** 문제가 발생했습니다. 조사 결과, CoreDNS 자체에는 문제가 없었지만 CoreDNS가 질의하는 **VPC DNS Resolver가 ENI PPS 한도에 걸려 패킷을 드롭**하고 있어 간헐적으로 응답 지연이 생긴 것이었습니다.

*사례 2*: 다른 사례로, EKS에서 **Aurora DB 클러스터**를 사용하던 중 읽기 부하 분산이 고르지 않아 일부 리더 노드에 세션이 몰리는 현상이 발견되었습니다. 분석 결과, **Aurora Reader 엔드포인트 DNS**가 여러 DB 리더 노드 IP를 1초 TTL로 라운드로빈 제공하는데, CoreDNS와 NodeLocal DNSCache가 이를 5초간 캐시하여 **5초 동안 동일한 리더 IP만 사용**하게 된 것이 원인이었습니다.
...
- **보강할 메트릭**: request_count_total, request_duration_seconds, cache_hits/misses, response_code_count_total, CPU/메모리.
- **TTL 권장치**: 서비스 레코드 30s, cache (success 30, denial 5-10).
- **모니터링**: kube-prometheus-stack 기본 대시보드 + Alertmanager 룰, 필요 시 NodeLocal DNSCache로 스케일-아웃.

## 1. CoreDNS 성능 모니터링에서 꼭 봐야 할 메트릭
...
:::warning TTL을 짧게 (10초 미만) 하면
stale IP 발생 가능성이 높아집니다.
:::

## 3. CoreDNS 모니터링 아키텍처 제안

1. **Prometheus Operator (kube-prometheus-stack)**
   - ServiceMonitor 가 kube-system/coredns 를 **:9153/metrics** 로 스크레이프
   - release=kps 에 포함된 Grafana k8s-coredns.json 대시보드 활용.
