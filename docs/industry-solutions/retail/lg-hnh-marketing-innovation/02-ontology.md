...
> Neptune(openCypher) 기반 지식 그래프(KG) 설계입니다. 데이터를 25개 유형(클래스)으로 나누며, 각 데이터 항목을 잇는 관계(edge)는 약 50만 개로 추정합니다. **3개 사업 부문(BU: Beauty·HDB·Refreshment)**의 데이터와 외부 4종 시그널을 함께 다룹니다.
