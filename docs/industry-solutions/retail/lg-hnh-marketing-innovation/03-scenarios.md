...
description: "GCC 14 → LG H&H 8 시나리오 압축. 모든 시나리오에 데이터 믹스 명시 (자사 + 외부 4종)"
---

# 8 시나리오 매핑

> GCC 14 시나리오 중 마케팅 혁신에 직결되는 8개 엄선. **모든 시나리오에 자사 데이터 + 외부 4종(소셜·기상·경제·경쟁사) 데이터 믹스 명시**.
...
## 2. GCC 14 → LG H&H 8 압축 매핑

| GCC 원본 | LG H&H 매핑 | 비고 |
|---|---|---|
| A 하이브리드 검색 | **S1** 자연어 시맨틱 검색 | 외부 리뷰 결합으로 강화 |
| B 페르소나 챗봇 | **S2** 5 부서 페르소나 챗봇 | 그대로 |
| C 인사이트 카드 | **S3** 카테고리·BU 인사이트 카드 | 외부 4종 결합으로 강화 |
| D 페르소나 매칭 | **S4 (병합)** | E와 병합 |
| E 클러스터링 | **S4 (병합)** | D와 병합 |
| F 룩어라이크 확장 | (제외) | PoC v2로 후속 확장 |
| G 캠페인 ROI | **S5** 옴니채널 캠페인 ROAS 시뮬 | 그대로 |
| H 권역 경쟁 지도 | (제외) | PoC v2로 후속 확장 |
| I 약관·가드레일 | **S8** 마케팅 동의·PII·미성년 가드레일 | 미성년 화장품 가드 추가 |
| J 외부 시그널 융합 | **S6** 외부 시그널 융합 | 4종으로 확장 |
| K 이상 행동 탐지 | (제외) | PoC v2 |
| L 결제·멤버십 매트릭스 | (제외) | PoC v2 |
| M 고객 통합 여정 | **S7** 옴니채널 회원 여정 | BU 가로지르는 행동 강조 |
| N 날씨 × 매출 | **S6 통합** | 외부 시그널의 일부로 |
