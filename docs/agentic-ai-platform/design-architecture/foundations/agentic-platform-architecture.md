...
Agentic AI 플랫폼은 에이전트가 모델과 도구를 호출해 작업을 수행하는 데 필요한 실행 환경입니다. 이 문서는 구성 요소를 **요청 처리에 참여하는 기능**과 **여러 기능에 공통으로 적용하는 운영 책임**으로 나눕니다.

요청 처리 기능은 **가속 컴퓨팅 인프라**, **모델 서빙**, **데이터·지식·메모리**, **에이전트 오케스트레이션**, **지능형 추론 라우팅**, **멀티 채널 노출**의 **6개 런타임 레이어**로 정리합니다. **관측성·평가**, **거버넌스·안전·주권**, **모델 라이프사이클**(FMOps)은 여러 레이어에 걸쳐 적용하므로 **3개 횡단 플레인**으로 구분합니다. 각 도전과제에 대한 상세 분석은 [기술적 도전과제](./agentic-ai-challenges.md) 문서를 참조하세요.
