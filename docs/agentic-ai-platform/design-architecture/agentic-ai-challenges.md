...
Agentic AI 시대를 맞아 기업이 가장 먼저 직면하는 질문은 *"가장 크고 비싼 LLM 하나만 쓰면 되지 않나?"*입니다. AI 분야의 주요 석학들은 이에 대해 일관된 경고를 보내고 있으며, 이 한계들이 곧 Agentic AI 플랫폼이 해결해야 할 도전과제의 근본 원인입니다.

### LLM의 구조적 한계 — 석학들의 관점

| 한계 | 핵심 주장 | 실용적 시사점 |
|------|----------|-------------|
| **추론과 계획의 부재** | Yann LeCun은 현재의 LLM이 세계를 이해하지 못하며, 계획 수립과 논리적 추론에 근본적 한계가 있다고 지적합니다. System 2 사고(느리고 의도적인 추론)는 모델 외부 시스템이 담당해야 합니다. | 에이전트의 **계획 수립은 워크플로우 엔진**(LangGraph)이, **논리 연산은 외부 도구**(Calculator API, Knowledge Graph)가 보완해야 합니다. |
| **환각은 구조적 특성** | Gary Marcus는 LLM의 환각이 버그가 아닌 구조적 특성이며, 트랜스포머 아키텍처의 TC⁰ 회로 복잡도 제약으로 복잡한 산술과 불리언 로직 처리가 본질적으로 불가능하다고 주장합니다. | 정확성이 요구되는 작업(요금 계산, 약관 검증)은 LLM에 맡기지 않고 **Knowledge Graph 팩트 체인 검증**과 **규칙 엔진**에 위임해야 합니다. |
| **LLM은 새로운 OS** | Andrej Karpathy는 "LLM은 새로운 운영체제"라는 비유를 통해, 모델 자체는 CPU에 불과하며 그 가치는 **주변 시스템**(메모리, 파일시스템, 도구, 네트워크)이 결정한다고 강조합니다. | 플랫폼의 가치는 모델이 아닌 **6-layer 아키텍처**(Gateway, Agent, Data, Observability 등) 전체에 있습니다. |
| **사전 학습 데이터의 고갈** | Ilya Sutskever 등 여러 연구자들은 인터넷 텍스트 기반의 사전 학습이 한계에 도달하고 있으며, 앞으로는 **합성 데이터와 도메인 특화 모델**의 시대가 올 것이라 전망합니다. | 범용 LLM에 의존하기보다 **자사 데이터로 파인튜닝한 도메인 SLM**을 육성하는 전략이 장기적으로 유리합니다. |

### 실용적 결론: 이질적 다중 모델 생태계

이러한 한계들을 종합하면, 기업은 단일 거대 LLM 의존에서 벗어나 **이질적 다중 모델(Heterogeneous Multi-model) 생태계**를 구축해야 합니다. 실제 연구에 따르면 MetaGPT, Open Operator, Cradle 등 에이전트 시스템에서 발생하는 LLM 호출의 **40%~70%는 적절히 파인튜닝된 SLM으로 대체 가능**합니다.
