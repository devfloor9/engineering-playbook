...
import { CoreCapabilities, LayerRoles, TenantIsolation, RequestProcessing } from '@site/src/components/ArchitectureTables';
...
flowchart LR
    subgraph AgentPod["Agent Pod"]
        RUNTIME["Agent Runtime<br/>워크플로우 엔진"]
...
flowchart TD
    subgraph Scaling["확장 전략"]
        subgraph AgentScale["Agent 확장"]
            ...
            EVENT_AGENT["Event-driven<br/>Queue Length"]
        end
...
:::tip 매니지드 모니터링
관리 오버헤드를 줄이기 위해 매니지드 모니터링 서비스를 활용하면 자동 스케일링과 고가용성을 확보할 수 있습니다.
:::
...
| 영역 | 필요 역량 | 설명 |
|------|----------|------|
| 컨테이너 오케스트레이션 | 관리형 Kubernetes | 노드 자동 프로비저닝, GPU 스케줄링 |
| 네트워킹 | Gateway API 지원 | 지능형 라우팅, mTLS, Rate Limiting |
| 모델 서빙 | LLM 추론 엔진 | PagedAttention, KV Cache 최적화 |
| Agent 프레임워크 | 워크플로우 엔진 | 멀티스텝 실행, 상태 관리, MCP/A2A |
| 데이터 레이어 | 벡터 DB + 캐시 | RAG 검색, 세션 상태 저장 |
| 관측성 | LLM 트레이싱 + 메트릭 | 토큰 비용 추적, Agent Trace 분석 |
| 보안 | 다층 보안 모델 | OIDC/JWT, RBAC, NetworkPolicy, 암호화 |
