...
- **GPU 드라이버 자동 관리**: NVIDIA GPU 드라이버를 AWS가 자동으로 설치하고 업데이트합니다. 별도의 GPU Operator나 NVIDIA Device Plugin 설치가 필요 없습니다.
- **NodeClass 자동 선택**: `default` NodeClass를 사용하면 Auto Mode가 최적의 AMI와 드라이버 버전을 자동 선택합니다.
- **운영 단순화**: GPU Operator 설치, CUDA 버전 관리, 드라이버 호환성 검증 등의 운영 부담이 제거됩니다.
- **GPU Operator 불필요**: Device Plugin, DCGM Exporter, MPS 등 GPU Operator 스택 전체를 AWS가 대체합니다.

**제한사항:**

- **MIG/Time-Slicing 불가**: Auto Mode의 NodeClass는 AWS 관리형(read-only)이므로 GPU 분할 설정이 불가능합니다. 소형 모델(7B~13B)에서 GPU 효율이 낮아질 수 있습니다.
- **DCGM 세밀 모니터링 제한**: GPU 레벨의 세밀한 메트릭(SM 활용률, NVLink 대역폭 등)을 DCGM Exporter로 수집할 수 없습니다. `nvidia-smi` 기본 메트릭만 사용 가능합니다.
...
llm-d는 EKS Auto Mode에서 빠르게 시작할 수 있지만, NVIDIA Dynamo의 전체 기능을 활용하려면 **Karpenter + GPU Operator** 환경으로 전환해야 합니다. Dynamo의 핵심 기능인 NIXL, KAI Scheduler, DCGM 세밀 모니터링, MIG 기반 Prefill/Decode 분리 등은 GPU Operator가 제공하는 저수준 GPU 제어에 의존하기 때문입니다.

#### 왜 Karpenter가 필요한가

Dynamo의 NIXL(NVIDIA Inference Xfer Library)은 GPU 간 직접 통신(NVLink/RDMA)을 사용하여 KV Cache를 초고속으로 전송합니다. 이를 위해 GPU Operator가 제공하는 NCCL/EFA 설정, NVIDIA Peer Memory 드라이버, GPUDirect RDMA 구성이 필수적입니다. EKS Auto Mode에서는 NodeClass가 AWS 관리형(read-only)이므로 이러한 저수준 GPU 네트워크 설정이 불가능합니다.

또한 KAI Scheduler는 DCGM Exporter가 수집하는 세밀한 GPU 메트릭(SM 활용률, 메모리 대역폭, NVLink 트래픽 등)을 기반으로 지능적인 워크로드 배치를 수행합니다. Auto Mode의 기본 메트릭만으로는 KAI Scheduler의 최적 스케줄링이 제한됩니다.
...
- [ ] **GPU Operator 설치**: `helm install gpu-operator nvidia/gpu-operator --namespace gpu-operator --create-namespace` 명령으로 GPU Operator를 설치합니다. Device Plugin, DCGM Exporter, MPS 등이 자동 배포됩니다.
...
- [ ] **Dynamo Platform 설치**: `helm install dynamo-platform nvidia/dynamo-platform --namespace dynamo-system --create-namespace` 명령으로 Dynamo Operator, KAI Scheduler, NIXL 런타임을 설치합니다.
- [ ] **DGDR CRD로 Disaggregated 워크로드 배포**: `DynamoGPUDisaggregatedRuntime` CRD를 사용하여 Prefill Worker와 Decode Worker를 정의하고, NIXL을 통한 초고속 KV Cache 전송을 활성화합니다.
...
- Karpenter + GPU Operator로 전환하여 MIG 기반 GPU 분할과 DCGM 세밀 모니터링을 활성화합니다.
- llm-d의 Disaggregated Serving으로 Prefill/Decode를 분리하여 처리량을 극대화합니다.
- 적합 대상: 중규모 프로덕션, 비용 최적화가 중요한 환경

**Phase 3: Karpenter + Dynamo (최대 성능, 대규모)**
- Dynamo Platform을 설치하여 NIXL 기반 초고속 KV Cache 전송과 KAI Scheduler를 활성화합니다.
- 3-tier KV Cache 관리(GPU→CPU→SSD)로 128K+ 긴 컨텍스트를 효율적으로 처리합니다.
- 적합 대상: 대규모 프로덕션 (GPU 16개+), 최대 처리량/최저 레이턴시 요구

:::caution 전환 시 주의사항
Phase 1에서 Phase 2로 전환할 때 클러스터 재생성이 필요할 수 있습니다. Auto Mode와 Karpenter 자체 관리는 동일 클러스터에서 혼용이 가능하지만, GPU Operator가 Auto Mode의 자동 GPU 관리와 충돌할 수 있으므로 별도 NodePool로 격리하는 것을 권장합니다.
:::
