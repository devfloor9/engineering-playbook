...
**EPP가 아닌 것**: 게이트웨이가 아니다(라우팅 결정만, 프록시는 Envoy). 모델 라우터가 아니다(within-model Pod 선택만). 레퍼런스 구현은 **llm-d Router**이며 EPP 코드는 llm-d로 통합되었고, GIE에는 InferencePool API와 Endpoint Picker Protocol만 남는다. EPP 호스트 게이트웨이는 **Envoy 계열만**: Istio, kgateway, Envoy Gateway, GKE Gateway, agentgateway.
...
Kong AI Gateway의 `ai-semantic-cache` 플러그인은 **임베딩 코사인 유사도로 의미가 유사한 요청의 "전체 응답"을 재사용**하는 L1 응답 캐시입니다(중복 LLM 호출 자체를 제거). 이는 vLLM Pod의 prefix KV 캐시 위치를 인지해 Pod를 고르는 **L2 KV-cache-aware 라우팅과 다른 계층**입니다(세 캐시 계층 비교: [Semantic Caching 전략](../../design-architecture/advanced-patterns/semantic-caching-strategy.md#2-캐시-계층-구분)).
