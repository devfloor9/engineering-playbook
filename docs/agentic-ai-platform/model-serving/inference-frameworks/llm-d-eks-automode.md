...
:::danger llm-d + DRA 사용 시 Auto Mode 제약 / Karpenter v1.14+ 지원
llm-d ModelService가 DRA (ResourceClaim) 방식으로 GPU를 요청하는 경우:
- **EKS Auto Mode**: 노드 프로비저닝이 동작하지 않습니다 (DRA 미지원). **Managed Node Group + Cluster Autoscaler** 구성 필요.
- **Self-managed Karpenter v1.14.0+**: DRA 지원 추가됨 (consumable capacity 및 partitionable devices 포함). v1.13 이하는 미지원.
