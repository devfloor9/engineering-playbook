...
본 문서는 HyperPod의 추론 라우팅이 [티어드 게이트웨이 아키텍처](../../reference-architecture/inference-gateway/tiered-gateway-architecture.md)의 어느 계층에 해당하는지, 풀스택 게이트웨이와 무엇이 다른지, 그리고 KV 캐시 구성·라우팅 전략·인스턴스 요건을 정리합니다. 대상 독자는 HyperPod 추론 엔드포인트를 평가·구성하는 플랫폼 엔지니어입니다.
...
[라우팅 전략 문서](../../reference-architecture/inference-gateway/routing-strategy.md)의 2-Tier 게이트웨이는 **L1 엣지 게이트웨이**(인증·Rate Limit·TLS)와 **L2 추론 라우팅**(KV-aware Pod 선택)을 별도 컴포넌트로 분리합니다. HyperPod Intelligent Router는 이 중 **L2만** 담당합니다.
...
### 관련 문서 (내부)
- [티어드 게이트웨이 아키텍처](../../reference-architecture/inference-gateway/tiered-gateway-architecture.md) — Tier 1/Tier 2 게이트웨이 계층 정의
- [추론 게이트웨이 라우팅 전략](../../reference-architecture/inference-gateway/routing-strategy.md) — L2 옵션 비교(EPP vs HyperPod vs Dynamo), 멀티리전 주의
