...
|------|-------|---------------|
| **아키텍처** | Aggregated + Disaggregated 모두 지원 | Disaggregated 중심 (Aggregated도 지원) |
| **KV Cache 라우팅** | Prefix-aware 라우팅 | Prefix-aware + NIXL GPU 간 전송 |
| **KV Cache 전송** | Pod 간 네트워크 전송 | NIXL (NVLink/RDMA 초고속 전송) |
...
:::tip 마이그레이션 경로
llm-d로 시작해서 규모가 커지면 Dynamo로 전환하는 것이 현실적입니다. 둘 다 vLLM을 백엔드로 사용하고, llm-d의 Disaggregated Serving 경험이 Dynamo 전환 시 도움이 됩니다. 핵심 차이는 Dynamo의 NIXL(GPU 간 초고속 KV 전송)과 KAI Scheduler(SLA 기반 오토스케일링)입니다.
:::
