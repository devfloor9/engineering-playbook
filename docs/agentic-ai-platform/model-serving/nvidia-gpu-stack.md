...
- **GPU Operator** (드라이버): GPU를 Kubernetes에 연결하는 기반 레이어. 드라이버 설치, 컨테이너 툴킷, 디바이스 플러그인, MIG 매니저를 DaemonSet으로 관리합니다.
...
MIG는 H100, A100 같은 최신 GPU를 최대 7개의 독립적인 GPU로 분할합니다. 각 MIG 인스턴스는 독립된 메모리, 캐시, SM(Streaming Multiprocessor)을 가지므로 워크로드 간 간섭 없이 안정적인 성능을 보장합니다.
...
Time-Slicing은 시간 기반으로 GPU 시간을 분할하여 여러 Pod이 동일 GPU를 공유합니다. MIG와 달리 모든 NVIDIA GPU에서 사용할 수 있지만, 컨텍스트 스위칭 오버헤드가 발생합니다.
...
Dynamo는 Prefill(프롬프트 처리)과 Decode(토큰 생성)를 분리하는 **Disaggregated Serving** 아키텍처를 채택합니다.
...
|------|-------|---------------|
| **아키텍처** | Aggregated (단일 워커) | Disaggregated (Prefill/Decode 분리) |
| **KV Cache 라우팅** | Prefix-aware 라우팅 | Prefix-aware + NIXL GPU 간 전송 |
| **KV Cache 전송** | 없음 (로컬 캐시만) | NIXL (NVLink/RDMA 초고속 전송) |
| **스케줄링** | 자체 스케줄러 | KAI Scheduler |
| **오토스케일링** | KEDA 연동 | KAI Scheduler 내장 |
| **vLLM 백엔드** | 지원 | 지원 (SGLang, TRT-LLM도 지원) |
| **Kubernetes 통합** | Gateway API 기반 | Operator + CRD (DGDR) |
| **복잡도** | 낮음 -- 기존 vLLM에 라우터 추가 | 높음 -- 전체 서빙 스택 교체 |
| **성능 향상** | Prefix hit로 TTFT 단축 | Disaggregated로 최대 7x 처리량 |
| **성숙도** | v0.3+ (초기) | v1.0 GA (2026.03) |

### 왜 함께 쓰기 어려운가

둘 다 **요청을 어떤 백엔드로 보낼지 결정하는 라우터** 역할을 합니다. 두 라우터를 직렬로 연결하면 llm-d의 KV cache 라우팅 결정을 Dynamo가 다시 덮어쓰게 되어 의미가 없습니다.

```text
llm-d 단독:    Client -> llm-d Router -> vLLM-1, vLLM-2, vLLM-3
Dynamo 단독:   Client -> Dynamo Router -> Prefill Workers -> (NIXL) -> Decode Workers
이중 라우팅:   Client -> llm-d -> Dynamo -> Workers  <- 라우팅 충돌, 비효율
```
...
| Prefill/Decode 독립 스케일링 필요 | **Dynamo** |
| 긴 컨텍스트 (128K+) 워크로드 | **Dynamo** (NIXL + 3-tier KV cache) |
| 빠른 도입, 낮은 운영 복잡도 | **llm-d** |

:::tip 마이그레이션 경로
llm-d로 시작해서 규모가 커지면 Dynamo로 전환하는 것이 현실적입니다. 둘 다 vLLM을 백엔드로 사용하므로 모델 서빙 레이어는 재사용할 수 있습니다. llm-d -> Dynamo 전환 시 Prefill/Decode 분리와 NIXL 설정이 주요 작업입니다.
:::
