...
상세 구현과 평가 결과는 [라우팅 전략 — Request Cascading: 지능형 모델 라우팅](./inference-routing/routing-strategy.md#request-cascading-지능형-모델-라우팅)을 참조하세요. 의미 기반 캐시와의 관계는 [Semantic Caching 전략](../design-architecture/advanced-patterns/semantic-caching-strategy.md)에서 다룹니다.
...
각 계층의 히트율을 어떻게 높이고 어디서 측정하는지를 통합한 의사결정 프레임은 [캐시 히트 전략](./inference-optimization/cache-hit-strategy.md)에서 다룹니다. Semantic 캐시의 임계값 설계는 [Semantic Caching 전략](../design-architecture/advanced-patterns/semantic-caching-strategy.md)을, Prefix 캐시 효과는 [KV Cache 최적화](./inference-optimization/kv-cache-optimization.md)를 참조하세요.
