...
:::info Karpenter v1.14+ (GA since v1.0)
Karpenter는 v1.0부터 GA 상태이며, 본 문서의 모든 예제는 `karpenter.sh/v1` API를 사용합니다. 현재 최신 버전은 v1.14 (2026-07-11 릴리스, DRA 지원 포함)입니다.
:::
...
:::info DRA 노드 프로비저닝 호환성 (2026.07 갱신)

| 노드 프로비저닝 | DRA 호환 | 비고 |
|---|---|---|
| **Managed Node Group** | ✅ 지원 | 권장 (모든 버전) |
| **Self-Managed Node Group** | ✅ 지원 | 수동 구성 필요 |
| **Karpenter v1.14.0+** | ✅ 지원 | 2026-07-11 릴리스부터 DRA 공식 지원 |
| **Karpenter v1.13 이하** | ❌ 미지원 | ResourceClaim 있는 Pod를 skip ([PR #2384](https://github.com/kubernetes-sigs/karpenter/pull/2384)) |
| **EKS Auto Mode** | ❌ 미지원 | 내부 Karpenter 기반이나 버전 정책 별도 |
:::
