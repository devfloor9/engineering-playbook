...
이 문서는 DRA를 프레임워크 관점에서 다룹니다 — 핵심 API 모델, DRA가 다루는 리소스 유형, 도입 판단 기준과 전제 조건. EKS GPU 환경에서 DRA를 실제로 활성화하는 파라미터(Karpenter `ignoreDRARequests`, NVIDIA DRA 드라이버 3계층 설정)는 [GPU 리소스 관리](./gpu-resource-management.md)에서 다룹니다.
...
가장 성숙한 활용 분야입니다. NVIDIA DRA 드라이버는 **GPU 할당**과 **ComputeDomain** 두 서브시스템으로 구성되며, GPU 할당 서브시스템은 MIG 파티션의 동적 생성·할당 같은 Device Plugin으로 불가능한 기능을 제공합니다. EKS에서의 활성화 파라미터와 Karpenter 조합은 [GPU 리소스 관리](./gpu-resource-management.md#dra-스택-전체-파라미터-3계층)를 참조하세요.
...
| 노드 오토스케일링 | Karpenter v1.14.0+ (`ignoreDRARequests=false`) 또는 MNG + Cluster Autoscaler — 상세는 [GPU 리소스 관리](./gpu-resource-management.md#karpenter-dra-활성화-파라미터-v1140) |
...
| EKS Auto Mode 사용 중 | 현재 DRA 불가 — 내부 Karpenter가 v1.14 미만. [GPU 리소스 관리](./gpu-resource-management.md#노드-프로비저닝-호환성) 참조 |
...
### 관련 문서 (내부)
- [GPU 리소스 관리](./gpu-resource-management.md) — EKS GPU 환경의 DRA 활성화 파라미터·Karpenter 조합·선택 가이드
- [EKS GPU 노드 전략](./eks-gpu-node-strategy.md) — DRA 워크로드를 위한 노드 프로비저닝 전략
- [NVIDIA GPU 스택](./nvidia-gpu-stack.md) — GPU Operator·MIG·Time-Slicing 상세
