...
:::info 참고 처리량 측정 조건
위 "참고 처리량" 수치는 Llama 2 계열 기준 BF16, Batch=1, Prompt 512 / Output 128 토큰, vLLM v0.18 구성에서의 내부 측정 예시값입니다. 실제 처리량은 모델, 양자화(FP8/INT8/AWQ), Batch 크기, 시퀀스 길이, TP/PP 구성에 따라 크게 달라지므로 배포 전 자체 벤치마크로 검증하세요.
:::
...
**NVIDIA Dynamo**는 데이터센터 규모의 LLM 추론을 최적화하는 오픈소스 프레임워크입니다. vLLM, SGLang, TensorRT-LLM을 백엔드로 지원하며, NVIDIA 공식 벤치마크 기준 Disaggregated Serving + Flash Indexer 조합에서 Aggregated 대비 **최대 7배 처리량 향상**을 보고합니다 (출처: NVIDIA Dynamo Blog, 워크로드·모델·GPU 구성에 따라 결과가 크게 달라집니다).
