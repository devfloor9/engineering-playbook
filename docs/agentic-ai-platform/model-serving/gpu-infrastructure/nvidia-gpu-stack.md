...
**v26.3.2 주요 신기능:** Blackwell(B200/GB200) 지원, HPC Job Mapping, CDMM(Confidential Computing), CDI(Container Device Interface)
:::
...
**NVIDIA Dynamo**는 데이터센터 규모의 LLM 추론을 최적화하는 오픈소스 프레임워크입니다. vLLM, SGLang, TensorRT-LLM을 백엔드로 지원하며, NVIDIA 공식 벤치마크 기준 Disaggregated Serving + Flash Indexer 조합에서 Aggregated 대비 **최대 7배 처리량 향상**을 보고합니다 (출처: NVIDIA Dynamo Blog, 워크로드·모델·GPU 구성에 따라 결과가 크게 달라집니다). NVIDIA Run:ai(2024-12 인수)는 스케줄러 코어로 KAI Scheduler(Apache 2.0)를 사용합니다.

:::info Dynamo v1.1.x (2026.06 기준)
