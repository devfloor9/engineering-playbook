...
- [커스텀 모델 파이프라인 — 평가 파이프라인](../reference-architecture/model-lifecycle/custom-model-pipeline.md#평가-파이프라인): LoRA 어댑터 평가 매트릭스, A/B 테스트
...
각 Phase별 상세 구현 가이드는 [커스텀 모델 파이프라인 구축 가이드](../reference-architecture/model-lifecycle/custom-model-pipeline.md#phase별-구축-로드맵)를 참조하세요.
