...
인프라 메트릭의 시간 창을 요청에 붙인 값은 **동시 구간 노출 변수**입니다. 그것으로 개별 요청이 preempt됐다고 주장하지 않습니다. 같은 시간 창에서 무관한 요청이 이벤트를 일으켰을 수도 있습니다.
...
품질을 평가된 요청의 통과율로 정했다면 `delta = p_new - p_baseline`과 허용 저하폭 `margin`을 사전에 정의합니다. 품질 저하를 배제하는 **비열등성** 판단은 delta 신뢰구간의 하한이 `-margin`보다 높은지를 봅니다. 양방향 동등성이 목적이면 상·하한 모두를 사전에 정한 동등성 구간과 비교하는 별도 검정을 설계합니다([통계 방법론](https://lakens.github.io/statistical_inferences/09-equivalencetest.html)).
...
상관 탐색에서는 모델, 과제 유형, 템플릿, 입력 길이, 턴 위치, 시간·부하를 고려합니다. 프롬프트 버전 자체가 처치라면 그것을 층화하여 처치 대비를 없애지 않습니다. 같은 과제·입력 집합에서 구·신 버전을 비교하고 다른 조건을 맞춥니다. 입력 토큰 수가 같다는 사실도 입력 내용이나 난도가 같다는 뜻은 아닙니다.
