...
TTFT는 Time to First Token, ITL은 Inter-Token Latency, TPOT는 Time per Output Token입니다. 토큰 간 간격 히스토그램과 요청별 평균 TPOT 히스토그램은 가중치가 다릅니다. 동일한 이름의 대시보드 패널에 혼합하지 않습니다.
...
결정론적 검사는 가능한 대상 전체에 적용하고 judge는 예산 내에서 표본 추출합니다. 예시 표본율 2%와 템플릿별 하드캡을 함께 사용하면 실제 포함 확률이 시간·부하에 따라 달라집니다. 표본 선정 규칙과 포함 확률을 저장하고, 층별 비교나 적절한 가중치를 사용합니다. 단순히 채점된 요청을 전체 트래픽의 무작위 표본으로 간주하지 않습니다.
