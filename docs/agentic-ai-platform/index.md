...
- [기술적 도전과제](./design-architecture/agentic-ai-challenges.md) — 핵심 과제 이해
- [EKS 기반 해결방안](./design-architecture/agentic-ai-solutions-eks.md) — Kubernetes 솔루션
- [플랫폼 아키텍처](./design-architecture/agentic-platform-architecture.md) — 전체 아키텍처 설계

### [모델 서빙 & 추론 인프라](./model-serving/index.md)

- [GPU 리소스 관리](./model-serving/gpu-resource-management.md) — MIG, Time-Slicing 등 GPU 리소스 설정
...
- [llm-d 분산 추론](./model-serving/llm-d-eks-automode.md) — Kubernetes 네이티브 분산 추론 (Auto Mode & Karpenter)
...
- [Ragas / DeepEval 평가](./operations-mlops/ragas-evaluation.md) — RAG 파이프라인 품질 평가
...
3. [플랫폼 아키텍처](./design-architecture/agentic-platform-architecture.md) - 아키텍처 설계
4. [GPU 리소스 관리](./model-serving/gpu-resource-management.md) - GPU 인프라 설정
5. [Inference Gateway](./gateway-agents/inference-gateway-routing.md) - 동적 라우팅 구성
6. [Agent 모니터링 & 운영](./operations-mlops/agent-monitoring.md) - 운영 체계 구축

**GenAI 애플리케이션 개발 경로:**

1. [vLLM 모델 서빙](./model-serving/vllm-model-serving.md) - 기본 모델 서빙
2. [Inference Gateway](./gateway-agents/inference-gateway-routing.md) - Bifrost 라우팅
3. [Milvus 벡터 DB](./gateway-agents/milvus-vector-database.md) - RAG 데이터 레이어
4. [Kagent Agent 관리](./gateway-agents/kagent-kubernetes-agents.md) - 에이전트 배포
5. [Ragas / DeepEval 평가](./operations-mlops/ragas-evaluation.md) - 품질 평가
:::
