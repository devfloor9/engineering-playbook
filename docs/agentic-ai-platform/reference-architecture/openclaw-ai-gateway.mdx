...
| 문서 | 내용 | 관계 |
|------|------|------|
| [Inference Gateway](../reference-architecture/inference-gateway-routing.md) | Kgateway 기반 라우팅 | 이론적 기반 |
| [Agent 모니터링](../operations-mlops/agent-monitoring.md) | Langfuse/LangSmith 모니터링 | 모니터링 이론 |
...
위 구성은 LiteLLM `cache: true` + Redis 기반의 실전 예시입니다. 유사도 임계값 선택, 캐시 키 설계(멀티테넌트 namespace), PII 안전 처리, 관측성 지표 등 전반적인 설계 원칙은 [Semantic Caching 전략](../design-architecture/semantic-caching-strategy.md) 문서를 참조하세요.
:::
...
:::tip 다음 단계
- 자체 호스팅 vLLM 추가 시: [llm-d 분산 추론](../model-serving/llm-d-eks-automode.md)를 참조하여 `Bifrost → llm-d → vLLM` 하이브리드 구성
- LiteLLM 대안 사용: Bifrost가 요구사항에 맞지 않을 경우 LiteLLM으로 대체 가능 (Python 기반, 동일한 OpenAI-compatible API)
- 벡터 검색 RAG 추가: [Milvus 벡터 DB](../operations-mlops/milvus-vector-database.md) 참조
- 에이전트 평가: [Ragas 평가](../operations-mlops/ragas-evaluation.md)로 응답 품질 측정
:::
