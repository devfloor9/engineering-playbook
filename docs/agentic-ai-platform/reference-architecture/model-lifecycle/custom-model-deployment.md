...
- **GLM-5.1**: GLM-5 동일 아키텍처에서 코딩/에이전틱 특화 post-training 수행
- **744B MoE (40B active)**: 256 experts 중 토큰당 8개 활성화
- **HuggingFace**: `zai-org/GLM-5.1-FP8` (BF16: `zai-org/GLM-5.1`)
- **라이선스**: MIT License
- **컨텍스트**: 200K 토큰 지원
- **성능** (모델 카드 기준): 
  - LiveBench Agentic Coding 55.00점
  - SWE-bench Verified 77.8% (GPT-4.1 54.6%)
...
:::info 자동 모델 분기
LLM Classifier를 사용하면 클라이언트가 단일 엔드포인트(`/v1`)로 요청하고, 프롬프트 내용에 따라 자동으로 SLM/LLM이 선택됩니다. 단순 요청은 Qwen3-4B(L4 $0.3/hr)로, 복잡한 요청(리팩터, 아키텍처, 설계 등)은 GLM-5 744B(H200 $12/hr)로 라우팅됩니다. 구성은 [추론 게이트웨이 배포: LLM Classifier](../inference-gateway/setup/advanced-features#llm-classifier-배포)를 참조하세요.
:::
...
| 최소 GPU | H200 8개 (1,128GB) 또는 B200 8개 (1,440GB) |
...
| p6-b200.48xlarge | B200×8 | 1,432GB | ✅ | ✅ (불필요) | ~$35/hr | ✅ 여유 |
...
:::info Auto Mode GPU 인스턴스 지원 (2026-04-10 업데이트)
EKS Auto Mode는 2026-04-10부터 p5en, p6-b200, p6-b300, p5e, trn2 등 최신 가속 인스턴스를 공식 지원합니다. p5/p5en/p6 모두 Auto Mode에서 사용 가능합니다.
:::
...
GLM-5 (`glm_moe_dsa` 아키텍처)는 vLLM v0.19.0+ (2026-04-03 릴리스)부터 표준 이미지에서 지원됩니다. 2026-07 기준 최신 안정 버전은 v0.25.1이므로 **커스텀 이미지 빌드 불필요**합니다.

```bash
# GLM-5 지원 버전 확인
# vLLM v0.19.0 (2026-04-03): GLM-5 GlmMoeDsaForCausalLM 지원 추가
# transformers 5.x 이상 포함
```

:::info 2026년 2월 이전 배포 시에만 커스텀 이미지 필요
GLM-5 출시 직후(2026년 2월)에는 vLLM v0.15~v0.17에 GLM-5 지원이 없어 transformers main 브랜치 설치 커스텀 이미지가 필요했으나, v0.19.0+ 표준 이미지는 즉시 사용 가능합니다.
:::

#### 권장 이미지 (2026-07 기준)

```dockerfile
# 표준 vLLM 이미지 사용 (커스텀 빌드 불필요)
FROM vllm/vllm-openai:v0.25.1

# FP8 Tensor Core 최적화 (H100/H200/B200)
...
| 6. Ray 모드 | `--distributed-executor-backend ray` | ❌ Kubernetes + Ray 통합 복잡 | Ray cluster 구성 필요 (v0.6+ 플래그는 --engine-use-ray 제거됨) |
...
- [ ] EKS 클러스터 생성 (Standard Mode, Kubernetes 1.35)
...
## 배포 검증 결과 (GLM-5.1 사례, 2026-04-03)
