...
:::caution 핫스왑 주의사항
vLLM은 요청 시점에 어댑터를 GPU 메모리로 로드합니다. `--max-loras`보다 많은 어댑터를 사용하면 **스왑 지연 시간**(수백ms)가 발생합니다. 자주 사용하는 어댑터 수에 맞게 `--max-loras`를 설정하세요.
:::
...
:::tip ROI 계산
위 비용 분석 표(70:30 트래픽 가정)를 기준으로 Cascade 도입 시 월 $5,880(연간 $70,560)을 절감할 수 있습니다. 설정에 소요되는 시간은 1-2일 수준입니다. 실제 절감액은 트래픽 구성·복잡도 분포·품질 기준에 따라 달라집니다.
:::
...
- 비용: ~$0.805/hr (On-Demand), ~$0.19-0.49/hr (Spot, 지역별)
- 8B 모델 서빙에 충분한 사양
:::
...
:::tip 투자 대비 효과 (ROI)
Phase 4까지 완료 시(가상 시나리오, 아래 전제에 따라 상이):
- **COBOL→Java 전환**: 10,000 모듈 x 1.5시간 절감 = **15,000시간 절감** (~$750K, 시간당 인건비 가정)
- **LoRA 학습 비용**: $2,000 (1회)
- **월 운영 비용 절감**: $5,880 (Cascade 도입 효과)
- **ROI: 약 375배** (절감액 ~$750K ÷ 1회 학습 비용 $2,000 기준)
:::
