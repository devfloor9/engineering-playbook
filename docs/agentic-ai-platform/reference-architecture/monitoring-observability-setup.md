...
Langfuse를 kgateway 통합 NLB의 `/langfuse` 경로로 서빙하는 설정입니다. 상세한 HTTPRoute YAML은 [게이트웨이 구성 가이드](./inference-gateway-setup.md#23-langfuse-sub-path-라우팅-urlrewrite)를 참조하세요.
...
상세 HTTPRoute YAML은 [게이트웨이 구성 가이드](./inference-gateway-setup.md#23-langfuse-sub-path-라우팅-urlrewrite)를 참조하세요.
