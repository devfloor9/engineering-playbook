...
reading_time: 1
tags: [llm-classifier, inference-extension, epp, inferencepool, cloudfront, waf, semantic-caching, 'scope:impl']
sidebar_position: 2
---
...
:::info 개념 및 설계 원칙
Semantic Caching의 개념, 유사도 임계값 설계, 캐시 키 구조, 관측성 전략은 [Semantic Caching 전략](../../../design-architecture/advanced-patterns/semantic-caching-strategy.md)을 참조하세요. 이 섹션은 실제 구현을 위한 도구 비교와 배포 설정을 다룹니다.
:::
...
| **Bifrost + Redis** | OSS (Apache 2.0) + OSS Redis | Redis | Go 기반 저지연, 라우팅 룰로 캐시 키 커스터마이징, 기존 Bifrost 배포 재사용 | Semantic Cache 자체는 플러그인/사이드카로 직접 구성 필요 | [Bifrost 문서](https://docs.getbifrost.ai/) |
...
cache = SemanticCache(
    ...
    distance_threshold=0.15,  # 1 - similarity (0.85 similarity ≈ 0.15 distance; 기본값 0.1)
)
...
**방법 B: 라우팅 룰 헤더 기반 분기** — Bifrost 라우팅 룰로 `x-cache-enabled: true` 헤더가 있는 요청만 Redis 경유
