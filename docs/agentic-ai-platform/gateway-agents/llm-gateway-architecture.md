...
> 작성일: 2026-03-16 | 수정일: 2026-04-04 | 읽는 시간: 약 20분
...
### Bifrost Gateway Mode 구성 (config.json)

Bifrost는 두 가지 운영 모드를 지원합니다: UI 모드와 Gateway 모드. Kubernetes 환경에서는 선언적 관리가 가능한 Gateway 모드를 권장합니다.

#### UI 모드 vs Gateway 모드 비교

| | UI 모드 | Gateway 모드 |
|---|---|---|
| 설정 방식 | 웹 UI (SQLite) | config.json (ConfigMap) |
| K8s 선언적 관리 | ❌ | ✅ |
| UI 대시보드 | ✅ | ✅ (읽기 전용) |
| 프록시 성능 | 동일 (100µs 미만) | 동일 |
| 설정 변경 | UI에서 즉시 | ConfigMap 변경 + PVC 초기화 |

#### config.json 구조

실제 동작이 확인된 config.json 포맷입니다:
...
```json
{
  ...
  "providers": {
    "openai": {
      ...
      "network_config": {"base_url": "http://glm5-serving.agentic-serving.svc.cluster.local:8000"}
    }
  },
  "plugins": [{
    "enabled": true,
...
**providers**
- `providers`는 **map** (배열 아님). key는 Bifrost 빌트인 provider 이름 (`openai`, `anthropic` 등)
- `keys`는 **배열**, `models`로 사용 가능한 모델 제한
- 요청 시 모델명은 `provider/model` 포맷 (예: `openai/glm-5`)

**OTel 플러그인**
- `trace_type`은 `otel` (not `genai_extension`)
- OTel collector_url은 Langfuse OTLP 전체 경로: `/api/public/otel/v1/traces`
- Authorization 헤더는 `Basic <BASE64(public_key:secret_key)>` 포맷

#### K8s 배포 패턴 (PVC + initContainer)
...
**배포 시 주의사항:**
- Bifrost는 `-app-dir /app/data`에서 config.json + SQLite 관리
- PVC 필요 (Pod 재시작 시 SQLite 유지)
- config.json 변경 시: PVC 데이터 삭제 → Pod 재생성 필요

```bash
# config.json 변경 후 PVC 초기화
kubectl delete pod -l app=bifrost -n ai-external
# initContainer가 새 config.json을 복사하고 Bifrost가 SQLite를 재생성
```

:::caution Gateway Mode 설정 변경

Bifrost는 config.json을 시작 시 1회 읽어 SQLite에 저장합니다. 이후에는 SQLite를 사용하므로, config.json 변경 시 PVC의 config.db를 삭제해야 새 설정이 반영됩니다.

**변경 절차:**
1. ConfigMap 업데이트
2. Pod 삭제 (PVC 데이터 자동 초기화됨)
3. initContainer가 새 config.json 복사
4. Bifrost가 SQLite 재생성

:::

#### Aider에서 Bifrost 사용

:::warning Bifrost provider/model 포맷과 OpenAI 클라이언트 호환성
Bifrost는 모델명을 `provider/model` 포맷으로 요구하지만, 이는 OpenAI API 표준이 아닙니다. 대부분의 OpenAI-compatible 클라이언트 (Aider, Cursor, OpenAI SDK)는 provider prefix를 자체적으로 처리하여 API 요청 시 제거합니다.

**호환 가능한 클라이언트**: Cline, Continue.dev (model 필드를 그대로 전달)
**우회 필요**: Aider (`openai/openai/glm-5` double-prefix 트릭)
**미지원**: Cursor (모델명 `/` 포함 거부)
...
aider --model openai/openai/glm-5 \
  ...
  --no-auto-commits  # 자동 커밋 비활성화 (선택)
```

:::caution 모델명 정규화
Bifrost는 모델명에서 하이픈을 제거하여 정규화합니다 (`glm-5` → `glm5`). vLLM의 `--served-model-name`을 정규화된 이름(`glm5`)으로 설정해야 합니다.
:::

모든 요청이 Bifrost → OTel → Langfuse로 자동 추적됩니다.

#### 트러블슈팅

**증상: UI에서 설정이 보이지 않음**
- 원인: config.json의 providers가 배열로 작성됨
- 해결: `"providers": [...]` → `"providers": {...}` (map으로 변경)

**증상: OTel trace가 Langfuse에 도착하지 않음**
- 원인: `trace_type: "genai_extension"` 사용
- 해결: `trace_type: "otel"` 사용

**증상: Langfuse가 403/401 에러 반환**
- 원인: Authorization 헤더 포맷 오류
- 해결: `Basic <BASE64(public_key:secret_key)>` 포맷 확인
