...
EKS Hybrid Nodes는 티어드 가격 모델을 적용하며, 월 누적 vCPU-hours에 따라 단가가 낮아집니다.
...
> **참고**: 하이브리드 요금은 보유 하드웨어 비용에 추가로 부과되는 관리 비용입니다.
...
```
단일 노드 (24시간 × 30일 × 224 vCPU = 163,520 vCPU-hr/월):
- 163,520 vCPU-hr × $0.020 = ~$3,270/월·노드

10개 노드 (1,635,200 vCPU-hr/월 총합):
- 첫 576,000 × $0.020 = $11,520
- 나머지 1,059,200 × $0.014 = $14,829
- 총합: ~$26,349/월 (10노드)
- 노드당 평균: ~$2,635/월

> **참고**: 위 금액은 EKS 하이브리드 관리 비용만이며, 하드웨어 구매·전력·공간 비용은 별도입니다.
```

:::tip 티어드 가격의 장점
대규모 배포 시 누적 vCPU-hours가 증가하면서 실효 단가가 크게 낮아집니다. 위 예시에서 10노드는 1티어를 넘어 2티어 단가(40% 절감)를 적용받습니다. 다음 최적화 방안도 병행하면 효과적입니다:
...
:::info DRA란?
동적 리소스 할당(DRA)은 Kubernetes 1.26에서 도입되어 **1.34에서 코어 GA**(`resource.k8s.io/v1`, 기본 활성화)된 기능으로, Pod이 요청할 때 GPU, NPU, 특수 가속기 등의 리소스를 동적으로 할당할 수 있게 합니다. 이는 기존의 정적 리소스 할당 방식을 개선하여 더 효율적인 리소스 활용을 가능하게 합니다.
:::
