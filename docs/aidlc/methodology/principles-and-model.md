...
이 가이드는 주·월 단위 개발 반복과 AIDLC가 제안하는 시간·일 단위 반복을 비교합니다. AIDLC에서는 AI가 요구사항, 작업 분해, 코드, 테스트를 제안하고, 사람이 정해진 검토 지점에서 산출물을 확인합니다.

**AIDLC(AI-Driven Development Lifecycle)**는 이러한 협업을 구성하기 위해 AWS Labs가 제시한 방법론입니다. 작업을 어떻게 나누고, 어떤 산출물로 문맥을 전달하며, 어느 단계에서 검토할지에 초점을 둡니다.
...
AIDLC에서는 사람이 원하는 결과를 정하면 AI가 구현에 앞서 계획을 제안합니다. 검토자는 제안된 범위, 아키텍처, 의존성과 위험을 프로젝트의 업무 우선순위에 맞춰 확인합니다.

예를 들어 에이전트가 인증, 사용자 API, 프로필 저장을 별도 Unit으로 제안할 수 있습니다. 검토자는 코드가 생성되기 전에 불필요한 인터페이스를 빼거나 저장 방식을 바꿀 수 있습니다.
...
이 가이드의 **Loss Function**은 사람의 검토를 설명하기 위한 비유입니다. 검토자는 각 단계에서 AI가 만든 산출물을 요구사항과 승인 기준에 대조하고, 다음 작업에서 사용하기 전에 오류 수정을 요청합니다. 여기서 말하는 피드백은 산출물에 대한 검토이며, 모델을 학습시키는 연산을 뜻하지 않습니다.
...
산출물이 해당 단계의 승인 기준을 충족하면 다음 단계로 진행합니다. 충족하지 못하면 수정한 뒤 다시 검토합니다. 실행할 단계와 검토 깊이는 프로젝트 상황에 맞춰 조정합니다.
