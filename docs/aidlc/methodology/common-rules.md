...
Common Rules는 Kiro, Q Developer, Cursor, Cline, Claude Code, GitHub Copilot 등에서 공통으로 따를 질문·승인·검증·기록 절차를 정의합니다. `AGENTS.md`는 이런 지침을 프로젝트에 전달하는 파일입니다. 공통 절차를 적용하더라도 산출물의 품질은 도구와 모델별로 평가해야 합니다.
:::
