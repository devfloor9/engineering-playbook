...
4. **규제 요구사항**  
   응답에 사용한 구성을 추적하려면 프롬프트·모델 버전과 변경 기록을 연결해 두는 방법이 유용하다. 조직의 감사 정책에 이를 반영하되, 수집할 내용과 보존 기간은 관할 지역, 규제 대상 역할·시스템, 기록 유형에 적용되는 의무를 확인한 뒤 정한다. [NIST AI RMF GOVERN 1.1](https://airc.nist.gov/airmf-resources/playbook/govern/)도 적용되는 요구사항을 파악하고 문서화하도록 안내한다. 이 가이드가 모든 금융·의료·공공 시스템의 법적 의무나 보존 기간을 정하는 것은 아니다.
...
4. **빠른 롤백**: 예를 들어 “1분 이내 복구”라는 목표를 정하고 실제로 시험한다. 라벨·설정 변경부터 애플리케이션과 트래픽의 정상화까지 측정하며, 처리 중인 작업과 대체 프롬프트도 확인한다. 1분은 이 문서의 예시 목표다.
5. **감사 증빙**: 7년은 조직 보존 정책을 검토하기 위한 예시다. 공통 법정 기간으로 사용하지 않는다. [감사 증빙 정책](./governance-automation.md#감사-증빙)에서 기록별 보존 근거, 최소 수집 범위, 접근·삭제 정책과 법적 보존 의무(legal hold)를 확정한다.
