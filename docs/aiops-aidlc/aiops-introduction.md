...
:::tip AWS MCP Server (통합 서버)
2025년 11월 AWS는 기존 개별 MCP 서버들의 기능을 통합한 **AWS MCP Server**를 발표했습니다. 15,000+ AWS API에 대한 자연어 접근, Agent SOPs(Standard Operating Procedures) 기반 워크플로우, CloudTrail 감사 로깅을 단일 서버로 제공합니다. 전체 목록은 [AWS MCP Servers GitHub](https://github.com/awslabs/mcp)를 참조하세요.
:::
